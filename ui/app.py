...
def app(
	...
	if str(qc_task).strip().lower() == "all" and not qc_tasks_eff:
		st.error(
			"`--qc_task all` needs a readable **qc.json** whose root is a JSON object with task keys. "
			f"Found no tasks at: `{qc_config_path}`"
		)
		st.stop()
	...
	if participant_id is None:
		cohort_complete = (not qc_cohort_eff) or SessionManager.all_qc_cohort_pages_complete_for_tasks(
			qc_tasks_eff, qc_cohort_eff
		)
