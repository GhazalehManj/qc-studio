...
### Multiple tasks in `qc.json` (`denoised_bold_qc`, `t1_2_mni_qc`, `sdc_wf_qc`, `coreg_wf_qc`)

See **`xcpd_QC_guidelines.md`** for pass/fail criteria. `qc.json` defines **four** tasks matching the doc panels.

`qc.json` can define **several** tasks. You choose how to run QC-Studio:

- **One task per run** (default): pass the task key with **`--qc_task`** (e.g. `denoised_bold_qc`). The UI does not switch tasks mid-session.
...
- From `ui/` with **`xcpd_test.sh`**: `./xcpd_test.sh ../pipelines/qsiprep/qc.json sdc_wf_qc`, or `./xcpd_test.sh ../pipelines/qsiprep/qc.json all`, or set **`QC_TASK=all ./xcpd_test.sh`**.
